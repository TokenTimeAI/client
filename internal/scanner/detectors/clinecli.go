...
// ClineCLIDetector scans Cline CLI data (distinct from Cline VS Code extension)
type ClineCLIDetector struct {
	scanner.BaseDetector
	dataDir string
}

func NewClineCLIDetector() scanner.Detector {
	paths := []string{
		"~/.cline-cli",
		"~/.config/cline-cli",
		"~/.local/share/cline-cli",
	}
	return &ClineCLIDetector{
		BaseDetector: scanner.NewBaseDetector("cline_cli", "Cline CLI standalone tool", paths, 50),
	}
}

func (d *ClineCLIDetector) Detect(ctx context.Context) (bool, error) {
...
func (d *ClineCLIDetector) Scan(ctx context.Context, state scanner.SourceState) ([]scanner.ScanResult, scanner.SourceState, error) {
	if d.dataDir == "" {
...
func init() {
	scanner.Register(NewClineCLIDetector)
}
